defmodule Assignment2 do
  ...
  def start() do
    ...
    if(length(args) == 4) do
      [numNodes, topology, algorithm, failedNodesPercentage] = args
      {totalNodes, _} = Integer.parse(numNodes)
      {failedNodes, _} = Integer.parse(failedNodesPercentage)
      ...
      newTotal = failNodes(initiatedNodes, failedNodes)
      IO.puts("New total is #{newTotal} nodes")
      time1 = System.monotonic_time(:millisecond)
      startAlgorithm(algorithm, initiatedNodes, newTotal, time1)
      infiniteLoop()
    else
  ...
  def failNodes(nodes, percentage) do
    failedNodesNum = Integer.floor_div(percentage * length(nodes), 100)
    nodesToKill = Enum.take_random(nodes, failedNodesNum)
    Enum.map(nodesToKill, fn x -> GenServer.cast(x, {:updateLifeStatus, false}) end)
    length(nodes) - failedNodesNum
  end
  ...
  def startAlgorithm(algorithm, allNodes, total, startTime) do

    case algorithm do
      "gossip" ->
        startGossip(allNodes, total, startTime)

      "push-sum" ->
        startPushSum(allNodes, total, startTime)
  ...
  def receiveGossip(randomNeighbour, total, startTime) do
    isAlive = GenServer.call(randomNeighbour, {:getLifeStatus})
  ...
  def startPushSum(allNodes, total, startTime) do
    IO.puts("Starting push-sum algorithm")
    table = :ets.new(:table, [:named_table, :public])
    :ets.insert(table, {"count", 0})
    startNode = getStartNode(allNodes)
    receivePushSum(startNode, 0, 0, total, nil, startTime)
  end

  def getStartNode(nodes) do
    startNode = Enum.random(nodes)

    if GenServer.call(startNode, {:getLifeStatus}) do
      startNode
    else
      getStartNode(nodes)
    end
  ...
  def receivePushSum(node, s, w, total_nodes, senderNode, startTime) do
    {nodeID, counter, neighbours, weight, value, isAlive} = GenServer.call(node, {:getstate})
    ...
    if isAlive do
      ...
      if change < :math.pow(10, -10) and counter == 2 do
        ...
        if count == total_nodes do
          ...
          IO.inspect "Time it took to achieve convergence is #{totalTime} milliseconds"
          System.halt(1)
        end
      end
      ...
      state = {nodeID, counter, neighbours, newweight / 2, newvalue / 2, isAlive}
      GenServer.cast(node, {:updatestate, state})
      randomNode = Enum.random(neighbours)

      Task.start(__MODULE__, :receivePushSum, [
        randomNode,
        newvalue / 2,
        newweight / 2,
        total_nodes,
      ...
      sendPushSumAgain(senderNode, s, w, total_nodes, startTime)
    end
  end

  def sendPushSumAgain(senderNode, s, w, total, startTime) do
    neighbours = GenServer.call(senderNode, {:getneighbours})
    randomNode = Enum.random(neighbours)
  ...
  def handle_cast({:updateneighbours, new_neighbours}, state) do
    {nodeID, counter, neighbours, weight, s, isAlive} = state
    state = {nodeID, counter, neighbours ++ new_neighbours, weight, s, isAlive}
    {:noreply, state}
  end

  def handle_cast({:update, x}, state) do
    {_, counter, neighbours, weight, s, isAlive} = state
    state = {x, counter, neighbours, weight, s, isAlive}
    {:noreply, state}
  end

  def handle_cast({:updatesum, s}, state) do
    {x, counter, neighbours, weight, _, isAlive} = state
    state = {x, counter, neighbours, weight, s, isAlive}
    {:noreply, state}
  end
  ...
  def handle_cast({:increaseCount, value, total, startTime}, state) do
    ...
    state = {nodeID, counter + value, neighbours, weight, s, isAlive}
    {:noreply, state}
  end
  ...
  def handle_cast({:updateLifeStatus, newStatus}, state) do
    {nodeID, counter, neighbours, weight, s, _} = state
    state = {nodeID, counter, neighbours, weight, s, newStatus}
    {:noreply, state}
  end

  def handle_call({:getLifeStatus}, _from, state) do
    {_, _, _, _, _, isAlive} = state
    {:reply, isAlive, state}
  end
  ...
  def handle_call({:getcount}, _from, state) do
    {_, counter, _, _, _, _} = state

    {:reply, counter, state}
  end

  def handle_call({:getneighbours}, _from, state) do
    {_, _, neighbours, _, _, _} = state
